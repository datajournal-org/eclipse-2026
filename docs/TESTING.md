...
- **`stores/localEclipse`** — already describes the guessed place on the first frame; recomputes on every `userLocation` change; matches
...
| `privacy.spec.ts`         | After picking a location the URL still has no `lat`/`lon` (the contract from `stores/location`), `localStorage` holds it, a reload restores the located state, and clearing storage falls back to the (unstored) time-zone guess                                                                             |
