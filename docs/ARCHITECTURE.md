...
- **Starting guess:** the browser's own time zone, resolved through a compiled-in IANA zone → city table
  (`$lib/geoguess`, generated by `scripts/build-timezones.ts` from the files vendored under
  `scripts/data/` — see the README there). No network, no permission prompt, `Intl` only. It exists so the
  app opens on a real sky rather than on a prompt: the feature that distinguishes this app is the
  per-location simulation, and it used to sit two screens below the fold behind a button.
- **GPS:** browser `Geolocation` API.
- **Place search:** VersaTiles geocoder.
- **Map tap:** click on the map → reverse geocoding for the place name.
- A `Place` carries **where it came from** (`$lib/place`): `user` (chosen, or the debug override), `guess`
  (from the time zone) or `showcase` (the fixed fallback for a zone the eclipse misses). B0 labels the
  latter two, and **only `user` is ever persisted** — storage means "the reader told us this".
